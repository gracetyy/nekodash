...
move once, dispatches the explicit move pipeline (`process_move(from_pos, to_pos, direction, tiles_covered)`), snapshots the player's previous-best data at level load time, and opens the Level Complete overlay once `level_record_saved` fires.
...
| Subscribing to `level_record_saved` for overlay logic  | Level Coordinator ✅ |
| Building the Level Complete overlay params dict        | Level Coordinator ✅ |
| Triggering the Level Complete overlay                  | Level Coordinator ✅ |
...
func _connect_signals() -> void:
    ...
    # level_record_saved → Level Coordinator triggers level-complete overlay
...
scene remains loaded under the modal; passing node references across systems is still
...
| **Transitioning** | `level_record_saved` fired                                  | Overlay shown by SceneManager   | `SceneManager.show_overlay()` called; `LEVEL_COMPLETE_OVERLAY_DELAY_SEC` timer running; overlay shown after delay. Restart during this window re-enters Playing and skips overlay. |
...
| `get_next_level()` returns `null` (last level)                           | Passed as `null` in params; Level Complete overlay hides Next Level button                                                     |
| `level_record_saved` fires while overlay transition is already in flight | `SceneManager.show_overlay()` is idempotent; duplicate call is either ignored or queued (SceneManager's responsibility)        |
...
| LC-5  | `level_record_saved` triggers `SceneManager.show_overlay(SceneManager.Overlay.LEVEL_COMPLETE, ...)` exactly once per level.                                  |
...
| LC-9  | The level-complete overlay appears no sooner than `LEVEL_COMPLETE_OVERLAY_DELAY_SEC` after `level_record_saved` fires.                                       |
...
| **Scene Manager**      | `show_overlay(SceneManager.Overlay.LEVEL_COMPLETE, params)` — triggers modal presentation | Hard      |
