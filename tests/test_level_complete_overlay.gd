...
func test_level_complete_plain_scene_ready_keeps_celebration_stars_upright() -> void:
	# Arrange
	...
	assert_eq(star_1.rotation_degrees, 0.0)
	assert_eq(star_2.rotation_degrees, 0.0)
	assert_eq(star_3.rotation_degrees, 0.0)


func test_level_complete_overlay_params_keeps_celebration_stars_upright() -> void:
	# Arrange
...
func test_level_complete_overlay_textured_stars_preserve_asset_tint() -> void:
	# Arrange
	var overlay_scene: PackedScene = load("res://scenes/ui/level_complete_overlay.tscn")
	var overlay: CanvasLayer = overlay_scene.instantiate() as CanvasLayer
	var level_data: LevelData = load("res://data/levels/world1/w1_l1.tres") as LevelData
	...
	var content: Control = overlay.get_node("Content") as Control
	var star_1: Control = content.get_node("MarginContainer/ResultsCard/CardMargin/VBox/StarRow/Star1") as Control
	assert_eq(star_1.texture.resource_path, "res://assets/art/ui/stars/star_large_filled.png")


func test_level_complete_ribbon_title_stays_inside_ribbon() -> void:
	# Arrange
	var scene: PackedScene = load("res://scenes/ui/level_complete.tscn")
	var screen: Control = scene.instantiate() as Control
	add_child_autofree(screen)
	...
	var title_label: Label = ribbon.get_node("RibbonTitleLabel") as Label
	assert_lte(title_label.global_position.x + title_label.size.x, ribbon.global_position.x + ribbon.size.x)
	assert_gte(title_label.global_position.x, ribbon.global_position.x)
